...
export default function StreetNamesPage() {
  const streetNames = [
    "Methadone", "Done", "Wafers", "Dolophine", "Physeptone", "Amidone",
    "Synthetic Heroin", "Diesel", "Meth", "Medicine", "Jungle Juice",
    "Tang", "Cherry Juice", "Pink", "Red", "Liquid", "Liquid Handcuffs",
    "Muddy Waters", "Reds", "Double Trouble", "Keep Well"
  ];
    ...
    <>
      ...
      <header className="bg-white border-b-2 border-teal-200 shadow-sm">
        <div className="max-w-6xl mx-auto px-4 py-8">
          <div className="flex flex-col items-center gap-6 text-center">
            <h1 className="text-6xl font-bold text-teal-700">Methadone - Street Names</h1>
            <p className="text-2xl text-gray-600">Common Names & Variants</p>
            <Link href="/drug/methadone">
      ...
      <main className="max-w-6xl mx-auto px-4 py-12">
        <div className="bg-blue-100 border-l-4 border-blue-500 p-6 rounded-lg mb-8">
          <h2 className="text-2xl font-bold text-blue-900 mb-3">Street Names for Methadone</h2>
          <p className="text-gray-700">Methadone goes by many different names. Learning these can help identify use and have informed conversations about dangers.</p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 mb-12">
          {streetNames.map((name, idx) => (
            <div key={idx} className="p-4 bg-blue-50 border border-blue-200 rounded-lg font-semibold text-blue-900 text-center">
              {name}
            </div>
          ))}
        </div>
